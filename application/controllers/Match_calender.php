...
{
    public $token ='ec471071441bb2ac538a0ff901abd249' ;//'0461b86e0a58765cd651c9068e0ccd73';
    ...
    {
       $api_url  =  "https://rest.entitysport.com/v2/matches/?status=2&token=".$this->token."&pre_squad=true&per_page=20";
        $ch = curl_init();
        ...
        $cricketMatches = json_decode($result,true); //json_decode($result); 
       // print_r(gettype( $cricketMatches)); die;
        // foreach($cricketMatches['response'] as $matchs) { 
            foreach ($cricketMatches['response'] as $items) {
            foreach ($items as $item) {
                ...
                if($afterdate >= $time1)
                {
                    ...
                    else if(isset($item['winner_team']) && $item['winner_team'] !="" && $item['matchStarted'] =="1")
                    {
                        $match_status ="Result";
                        $timespell = "0000-00-00 00:00:00";
                    }    
