...
{
    public $apikey = '0461b86e0a58765cd651c9068e0ccd73';
    ...
    function send_sms($contact_no,$message){        
        ...
        $authKey = "76339d73b2e6dcf112ef60337c9a3cd2";
...
//      }   
    ...
    //echo "<pre>"; print_r($matchs);die();
        ...
        {   
            foreach ($matchs as $match) {
                ...
                $result=curl_exec($ch);
                curl_close($ch);
                $resp = json_decode(json_encode(json_decode($result)), True);   //echo "<pre>";print_r($id);die;          
    ...
    public function CashFree_token_post()
   {
     ...
     //  print_r($amount);echo"<br>"; print_r($orderId);die;
       // $orderId = "0". time();
        $mode = "PROD";
        if($mode == 'PROD'){
            $host = "https://api.cashfree.com";
            $appId = "13329005100b3583116863fbb92331";
            $secret = "76343440ae75ffd8caad043f1a780cf798f08257";
        } else {
            $host = "https://test.cashfree.com";
            $appId = "5170990d6ab6f4ac6466e8ac0715";
            $secret = "1e9a7ce0ef8f871e69dc27e34a15bd16c7e94401";
        }
   ...
   function update_livematch_status_get()
	{
		$response = $this->Webservice_model->get_livematch_status();
			...
			foreach ($response as $resp) {
		        ...
		        { 
		        	$data = array('time'=>"0000-00-00 00:00:00",
						"match_status" =>"Result",
						'matchStarted'=>'0',
						'match_status_note'=> $resp['response']['status_note']
					);
					$data1 = array('time'=>"0000-00-00 00:00:00",
						"match_status" =>"Result",
						'matchStarted'=>'0'
					);
					$this->db->where('match_id',$id);
	...
	function match_playing_eleven_get()
    {
       ...
       foreach($matchs as $match)
       {
             ...
             $api_url = "https://rest.entitysport.com/v2/matches/".$m_id."/squads?token=".$this->apikey."";
           ...
           $teama = $result_responce['response']['teama']['squads'];
           $teamb =  $result_responce['response']['teamb']['squads'];
           
            if(in_array('true', array_column($teama, 'playing11'))) { 
           ...
           { 
                  if($team['playing11'] =="true")
                    {
                       $p_id =  $this->Webservice_model->get_match_player_id($m_id,$team['player_id']);
